...
> **ReaLLM-Forge fork.** This fork adds tooling to run [ReaLLM-Forge](.) models (RoPE + GeGLU +
> RMSNorm, GPT-2 BPE) in this C engine, including on Android — see [`reallmforge/`](reallmforge/)
> and [`doc/reallmforge_to_llama2c.md`](doc/reallmforge_to_llama2c.md). An example model lives under
> [`models/smollm2_135M/`](models/smollm2_135M/): the tokenizer is committed, but the ~138 MB weights
> are **not in git** (GitHub's 100 MB limit; no LFS on this fork) — download them from the repo's
