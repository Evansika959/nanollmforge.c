...
#if !defined(NLF_DISABLE_GS64_DECODE)
        else if (GS == 64) {
            // Decode Q8 GEMV: one exact int32 dot per quantization group.
            // Widen EVERY int16 product before accumulation: even two -128
            // products can overflow int16. Two chains reduce dependencies.
            const int groups = n / 64;
            const float* ws = w->s + i * groups;
            for (int g = 0; g < groups; g++) {
                const int j = g * 64;
                int32x4_t a = vdupq_n_s32(0), b = vdupq_n_s32(0);
                for (int k = 0; k < 64; k += 32) {
                    int8x16_t vx0 = vld1q_s8(x->q + j + k);
                    int8x16_t vw0 = vld1q_s8(w->q + in + j + k);
                    int8x16_t vx1 = vld1q_s8(x->q + j + k + 16);
                    int8x16_t vw1 = vld1q_s8(w->q + in + j + k + 16);
#if defined(__ARM_FEATURE_DOTPROD)
                    a = vdotq_s32(a, vx0, vw0);
                    b = vdotq_s32(b, vx1, vw1);
#else
                    a = vpadalq_s16(a, vmull_s8(vget_low_s8(vx0), vget_low_s8(vw0)));
                    a = vpadalq_s16(a, vmull_s8(vget_high_s8(vx0), vget_high_s8(vw0)));
                    b = vpadalq_s16(b, vmull_s8(vget_low_s8(vx1), vget_low_s8(vw1)));
                    b = vpadalq_s16(b, vmull_s8(vget_high_s8(vx1), vget_high_s8(vw1)));
#endif
                }
                int32x4_t sum = vaddq_s32(a, b);
                int32x2_t pair = vadd_s32(vget_low_s32(sum), vget_high_s32(sum));
                int32_t dot = vget_lane_s32(pair, 0) + vget_lane_s32(pair, 1);
                val += ((float)dot) * ws[g] * x->s[g];
            }
        }
#endif
        else {
